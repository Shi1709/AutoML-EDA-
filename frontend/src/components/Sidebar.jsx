...
const steps = [
  ...
  { id: 2, title: "Data Cleaning", desc: "Handle missing values", icon: Eraser },
  { id: 3, title: "Correlation", desc: "Remove redundant features", icon: GitBranch },
  { id: 4, title: "Encoding", desc: "Transform categorical data", icon: Code },
  { id: 5, title: "Target Selection", desc: "Choose prediction target", icon: Target },
  { id: 6, title: "Train/Test Split", desc: "Divide dataset", icon: Split },
  { id: 7, title: "Model Training", desc: "Train ML models", icon: Play },
  { id: 8, title: "Compare Models", desc: "Evaluate performance", icon: BarChart3 },
...
const SidebarItem = ({ step, active, onClick }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 leading-tight">
        <p className={`text-sm font-medium ${active ? "text-blue-700" : "text-gray-800"}`}>
          {step.title}
        </p>
        <p className="text-xs text-gray-500">{step.desc}</p>
      </div>
      <Icon size={16} className={`${active ? "text-blue-600" : "text-gray-400"} mt-1`} />
    </button>
  );
};
